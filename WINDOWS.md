...
- [x] **Task runner** (`:task`, `:tasklast`, `<leader>mm` / `<leader>ml`) — `task_glue` runs `default_shell() -l -i -c "cd <cwd> && exec <command>"`. `cmd.exe` rejects POSIX-style dash flags; needs a per-shell dispatch (`/C` for `cmd.exe`, `-Command` for `pwsh.exe`, untouched for `bash`-family). The five task sources (npm / Justfile / cargo / Makefile / dotnet) and the parsed shell command line otherwise carry over.
- [x] **AI side pane** (`<leader>jc/jx/jo` + uppercase variants for path handoff) — same `default_shell() -l -i -c "exec <tool>"` shape in `side_terminal_glue`. Same per-shell dispatch fix applies.
- [x] **`shell_quote`** for task launching — POSIX single-quote wrapping. Windows needs the cmd.exe quoting variant (double-quote with `^` escapes for caret-interpreted chars). Trivial once the dispatch above lands; rates a `#[cfg]` switch on the helper.
...
- [ ] **`binvim-install` / `:install` — native Windows package managers (winget / scoop / choco).** The path-walking mechanism is already portable: `paths::find_on_path` splits on `;` and synthesises `.exe` / `.cmd` / `.bat` candidates on Windows, so cross-platform managers (`cargo`, `rustup`, `npm`, `go`, `pipx`, `pip`, `gem`, `dotnet`, `composer`, `nix`) get detected fine. What's missing is the Windows-native trio. `detect_managers` at `install.rs:461` doesn't probe `winget` / `scoop` / `choco`, and `Installer` (the `enum` at the top of `install.rs`) has no `Winget` / `Scoop` / `Choco` variants — so any tool whose only auto-install paths today are `Installer::Brew(_)` or `Installer::Apt(_)` falls through to `Choice::NoManager` on Windows even when winget would happily install it. Fix is two-file: add the three `Installer` variants (each carrying a package id), append the three names to the `detect_managers` candidate list, and populate the new variants on every `Tool` in `BUNDLES` that currently has a Brew/Apt entry but is available on winget/scoop/choco. README install table + binvim.dev's install table need matching version pins per the existing pattern.
