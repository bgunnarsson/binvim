...
- [x] **`binvim-install` / `:install` — native Windows package managers (winget / scoop / choco).** `Installer::Winget` / `Scoop` / `Choco` carry a package id each, and `detect_managers` probes all three. They come after every other installer in a `Tool`, so a macOS / Linux pick never changes (`windows_entries_never_change_a_non_windows_pick`), and on Windows winget wins when present. scoop runs through PowerShell, since it is a PowerShell shim, and only its `main` bucket is used. Eight tools gained a Windows path (clangd, clang-format, lua-language-server, marksman, zls, zig, elixir-ls, mix). lldb-dap, jdtls, google-java-format, kotlin-language-server and ktfmt have no package that puts them on `PATH` and stay `NoManager` on Windows. The ids and where each was confirmed are in `docs/plans/2026-09-23-installer-installs-through-winget-scoop-and-choco.md`.
