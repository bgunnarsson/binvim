...
- [x] **AI side pane** (`<leader>jc/jx/jo` + uppercase variants for path handoff) — the same `shell_launch`. Under cmd.exe the tool is resolved through `PATH` first, because cmd looks in the current directory (the project) before `PATH`.
