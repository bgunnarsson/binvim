...
| A buffer with no file name | A `[No Name]` buffer with unsaved changes is dumped every 4 s, and on SIGTERM / SIGHUP, under an `unnamed-<pid>-<secs>-<n>` key it's given on its first dump. The next launch says how many a crash left, and `:recover` opens each as a buffer with unsaved changes. It rewrites the dump under its own pid right away, so a second binvim's `:recover` can't take it too. A dump belonging to a running binvim isn't offered. The dump is removed by `:w {file}`, `:bd!`, closing all buffers and a clean quit. **Not covered:** internal path-less buffers (`display_name` set: `[Command Line]`, `[config defaults]`), which aren't your text. | `dump_key` / `recovery_key`, `write_recovery_now`, `refresh_recovery_snapshot`, `announce_unnamed_recovery`, `cmd_recover` (`app/recover_glue.rs`), `crash_unnamed_dumps` (`recover.rs`) | Tests: `a_no_name_buffer_keeps_one_unnamed_key_until_it_has_a_path`, `an_internal_buffer_has_no_recovery_key`, `only_parsed_unnamed_dumps_are_offered`, `unnamed_keys_are_this_process_s_and_never_repeat`. Checks (2026-09-23), in tmux against a release build with a scratch cache: a `:new` buffer's dump appeared within 5 s. After `tmux kill-session`, the relaunch said `1 unnamed buffer left by a crash`. With a second binvim running, `:recover` took only the crashed dump, and rewrote it under its own pid. `:w` removed it, `:bd!` removed another, a second `:recover` found nothing, and quitting the second binvim removed its dump. |
