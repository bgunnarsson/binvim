...
- **`dap.rs` + `dap/`** is the Debug Adapter Protocol client, structurally parallel to `lsp/` — same split into **`types`**, **`specs`** (adapter registry + workspace-root discovery + `$PATH` lookup), **`client`**, **`io`**, **`manager`**. `DapManager` owns the active debug session, the user's breakpoint table, and the receiver side of the reader-thread channel; adapter-agnostic, with specific debuggers (e.g. `netcoredbg`) declared only in `dap/specs.rs`.
...
Register the adapter in `dap/specs.rs` (binary lookup + workspace root discovery + launch config). Adapter-specific behaviour stays in `specs.rs`; `manager.rs` and the wire protocol in `types`/`io`/`client` are adapter-agnostic. There is no plugin system — like the LSP layer, every adapter is hard-wired.
