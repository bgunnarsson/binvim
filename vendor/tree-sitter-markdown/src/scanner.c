...
static bool parse_ordered_list_marker(Scanner *s, TSLexer *lexer,
    ...
    if (s->indentation <= 3 &&
        ...
        advance(s, lexer);
        // binvim: `lookahead` is a Unicode scalar, and the narrow `isdigit`
        // is undefined outside 0..=255 (glibc reads past its table). The
        // same change was proposed upstream in tree-sitter-markdown#252.
