...
| `:terminal` / `:term [cmd]` | Open the embedded PTY shell as a bottom split pane (vt100 / xterm escape parsing, 10k-row scrollback, full SGR colour + attrs, mouse-tracking forwarding). `Mode::Terminal` forwards keystrokes to the PTY; `Esc` drops into `Mode::TerminalNormal` for Vim-style navigation + selection (`h`/`j`/`k`/`l` move a reading-cursor, `v` enters Visual, `y` yanks the selection to the unnamed register + system clipboard, `Y` yanks the row, `i`/`a` re-enters Terminal mode, `<C-w>q` or `:q` closes the pane). Click inside the pane forwards to the PTY when the inner program asked for mouse tracking (DECSET 1000 / 1002 / 1003 / 1006 — htop, vim, less mouse mode, …), otherwise pulls focus into the shell. PTY auto-resizes with the host terminal. Default command is `$SHELL` (fallback `/bin/sh`); `:terminal htop` spawns that instead. |
