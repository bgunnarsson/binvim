...
- **Git gutter** — leftmost gutter column shows working-tree changes against the git index: Green `▎` for added lines, Yellow `▎` for modified, Red `▁` for the line just below a deletion. Refreshed on save, on buffer switch, and on initial open. Shells out to `git diff --unified=0` — works on any path inside a git repo with no extra setup.
