...
| `:terminal` / `:term [cmd]` | Open the embedded PTY shell as a fullscreen overlay (vt100 / xterm escape parsing, 10k-row scrollback, SGR colour, bold / italic / underline / reverse). `Mode::Terminal` forwards keystrokes to the PTY (xterm sequences for arrows / F-keys / Page / Home / End / Delete; Ctrl-letter → C0 control codes; Alt-prefix for Meta). `Esc` drops into `Mode::TerminalNormal` — editor bindings active again, `i` / `a` re-enters Terminal mode, `:q` closes the overlay. PTY auto-resizes on window resize. Default command is `$SHELL` (fallback `/bin/sh`); `:terminal htop` spawns that instead. |
