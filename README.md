...
- **Search** — `/` and `?` take Vim's regex syntax (`\(…\)`, `\|`, `\+`, `\{n,m}`, `\<` / `\>`, `\s` / `\d` / `\w`, `[…]`, `\zs` / `\ze`, `\v` / `\V`) with smart case — an upper-case letter makes it case-sensitive, `\c` / `\C` decide outright — and every match is highlighted in full. `*` / `#` search the word under the cursor as a whole word, `g*` / `g#` as bare text, and `n` / `N` take a count.
