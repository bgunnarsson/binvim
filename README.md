...
| `<space>i`  | Set up the toolchain for this language — opens `:install` preselected to the current buffer's language bundle (LSP + formatter + DAP), so you review and confirm with `y`. When a file's LSP or formatter is missing on open, a one-line hint points here (disable with `[install] prompt_on_open = false`) |
...
**`[install]`** — `prompt_on_open = true` (the default) is the first-run toolchain nudge: when you open a file whose language is missing its primary LSP or formatter (probed on `$PATH`), a one-line status hint names what's missing and points at `<leader>i`, which opens `:install` preselected to that language's bundle so you review and confirm with `y`. The hint fires at most once per language per session, is skipped for large files (which don't attach a server anyway) and for languages binvim can't auto-install (Razor's OmniSharp), and never nags about DAP adapters — only the LSP + formatter that make a language feel "set up." Set `prompt_on_open = false` to silence it; `<leader>i` still works on demand.
