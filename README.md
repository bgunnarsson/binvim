...
| Focus pane               | `<leader>df` / click on the pane | Enters `Mode::DebugPane`. Five tabs across the top: **Frames**, **Locals**, **Watches**, **Breakpoints**, **Console**. `Tab` / `Shift-Tab` (or `←`/`→`, or click the label) cycles between them. `1`-`5` jumps to a tab by number. `j`/`k`/`g`/`G` navigate within the active tab. `Enter` (or `Space`) on a Locals row expands the structured value. `Ctrl-Y`/`Ctrl-E` scrolls the body. `c`/`n`/`i`/`O` step without leaving the pane. `:` enters the command line. `Esc` returns to Normal. |
