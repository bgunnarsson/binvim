...
- **Git gutter + hunks** — leftmost gutter column shows working-tree changes against the git index: Green `▎` for added lines, Yellow `▎` for modified, Red `▁` for the line just below a deletion. Status line gains a `+A ~M -D` counter next to the branch name. `]h` / `[h` jump to next / previous hunk; `<leader>hp` previews the hunk under the cursor in a hover popup (three lines of surrounding context). Refreshed on save, on buffer switch, and on initial open. Shells out to `git diff --unified=0` — works on any path inside a git repo with no extra setup.
