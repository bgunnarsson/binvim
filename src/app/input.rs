...
impl super::App {
    ...
    fn handle_mouse_event(&mut self, ev: MouseEvent) {
        ...
        // Side-pane (`:claude` etc.) mouse handling lives in
        // side_terminal_glue, mirroring the bottom-pane and debug-pane
        // extractions.
        if self.handle_side_terminal_mouse_event(&ev, row, col) {
            return;
        }
        ...
        // File-tree sidebar clicks — extracted like the pane handlers above.
        if self.handle_file_tree_mouse_event(&ev, row, col) {
            return;
        }
