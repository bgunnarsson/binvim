...
impl App {
    ...
    pub fn run(&mut self) -> Result<()> {
        ...
        // Clean shutdown — remember the active buffer's cursor nvim-style.
        // Open, move, quit without a save — the next open still comes back
        // here (the content hash gates it against changed content).
        self.persist_active_cursor();
        // Persist the session so the next launch in this cwd can restore it.
        // Best-effort: errors don't block exit.
